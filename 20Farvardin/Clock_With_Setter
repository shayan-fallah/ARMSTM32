    ...
    /* USER CODE BEGIN 3 */
		...
		for(h = 0; h <= 24; h++)
		{
			for(m = 0; m <= 60; m++)
			{
				for(s = 0; s <= 60; s++)
				{
					if(HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_8) == 0) // second increase BUTTON
				{
					if (s == 60)	s = 0;
					else	s++;
					HAL_Delay(300);
				}
				if(HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_9) == 0) //minute increase  BUTTON
				{
					if (m == 60) m = 0;
					else	m++;
					HAL_Delay(300);
				}
				if(HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_6) == 0)	//hour increase		BUTTON
				{
					if (h == 24) h = 0;
					else	h++;
					HAL_Delay(300);
